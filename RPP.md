...
2. **Filament *Policies*** — 17 *policies* implementadas para controlo granular (`UserPolicy`, `EmployeePolicy`, `ContractPolicy`, etc.).
...
- **Serviços especializados:** lógica de negócio complexa encapsulada em *Services* (`GeneratePayrollService`, `CalculateExtraHoursService`, `DeductHourBankService`, `AttendanceCalculationService`, `VacationBalanceService`).
...
Validam componentes isolados como *middlewares* e utilitários de *dashboard* (por exemplo, `EnsureUtf8EncodingTest`).
...
**Automatização com *Observers*** — quando um contrato é criado, o sistema precisa de sincronizar a designação (*designation*) do funcionário para manter consistência. Esta lógica foi implementada com *Observers*, assegurando que alterações no contrato se refletem automaticamente no modelo `Employee`.
...
**Validação do Banco de Horas** — o sistema que valida licenças antes de descontar horas poderia originar problemas de N+1. A solução foi otimizar com *eager loading* correto das relações.
...
- **Automatização inteligente:** o padrão *Observer* automatiza a sincronização de dados entre modelos, reduzindo erros manuais.
